...
from pycoolc.ir.tac import (
    ...
    TACMethod, Call, Dispatch, StaticDispatch, New, IsVoid, GetAttr, Phi,
    Param,
)
...
class TestConstantPropagationTransfer:
    ...
    def test_static_dispatch_returns_unknown(self):
        ...
        instr = StaticDispatch(dest=Temp(0), obj=Var("self"), static_type="A", method="foo", num_args=0)
        result = analysis.transfer(env, instr)
    ...
    def test_binop_string_eq(self):
        ...
        instr = BinaryOp(dest=Temp(0), op=BinOp.EQ, left=Const("a", "String"), right=Const("a", "String"))
        result = analysis.transfer(env, instr)
...
class TestConstantFolding:
    ...
    def test_partial_fold_binop(self):
        ...
        cfg = build_cfg(method)
        result, changes = run_constant_propagation(cfg, params=["p"], fold=True)
