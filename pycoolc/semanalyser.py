...
class PyCoolSemanticAnalyser:
    ...
    def _init_collections(self, program_ast: AST.Program) -> None:
        ...
        self._classes_map, self._inheritance_graph = self._build_classes_map_and_inheritance_graph(
            self._program_ast
        )
    ...
    def _install_builtin_types_to_ast(program_ast: AST.Program) -> AST.Program:
        ...
        if not isinstance(program_ast, AST.Program):
            raise SemanticAnalysisError(f"Expected AST.Program, got {type(program_ast).__name__}")
    ...
    def _check_cyclic_inheritance_relations(self) -> None:
        ...
        seen = dict.fromkeys(self._classes_map, False)
    ...
    def _check_main_class(self) -> None:
        ...
        if main_method is None:
            raise SemanticAnalysisError("Class 'Main' must have a 'main()' method.")

        if len(main_method.param_types) > 0:
            raise SemanticAnalysisError("Method 'main' in class 'Main' must take no arguments.")
    ...
    def _check_method_overriding(self) -> None:
        ...
        for class_name, klass in self._classes_map.items():
            ...
            for feature in klass.features:
                ...
                parent_method = self._get_inherited_method(klass.parent, feature.name)
        ...
        match expr:
            ...
            case AST.NewObject(type=new_type):
                ...
                if new_type not in self._classes_map:
                    raise SemanticAnalysisError(f"Cannot instantiate undefined class '{new_type}'.")
                return new_type
            ...
            case AST.If(predicate=pred, then_body=then_expr, else_body=else_expr):
                ...
                if pred_type != BOOLEAN_CLASS:
                    raise SemanticAnalysisError(f"If predicate must be Bool, got '{pred_type}'.")
            ...
            case AST.WhileLoop(predicate=pred, body=body):
                pred_type = self._infer_type(pred, env)
                if pred_type != BOOLEAN_CLASS:
                    raise SemanticAnalysisError(f"While predicate must be Bool, got '{pred_type}'.")
    ...
    def _check_arithmetic(
        self, left: AST.AST, right: AST.AST, op: str, env: TypeEnvironment
    ) -> str:
        ...
        if left_type != INTEGER_CLASS:
            raise SemanticAnalysisError(f"Left operand of '{op}' must be Int, got '{left_type}'.")
        if right_type != INTEGER_CLASS:
            raise SemanticAnalysisError(f"Right operand of '{op}' must be Int, got '{right_type}'.")
